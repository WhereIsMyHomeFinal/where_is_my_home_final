...
public class LikeDealController {
	...
	@DeleteMapping("/{userIdx}/{dealIdx}")
	private ResponseEntity<?> deleteLikeDeal(@PathVariable("userIdx") int userIdx, @PathVariable("dealIdx") int dealIdx) {
		logger.info("deleteLikeDeal - 호출");
		HashMap<String, Integer> map = new HashMap<>();
		map.put("userIdx", userIdx);
		map.put("dealIdx", dealIdx);
		if (likeDealService.deleteLikeDeal(map)) {
			return new ResponseEntity<String>(SUCCESS, HttpStatus.OK);
		}
